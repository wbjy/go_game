...
type GateLogic struct {
	...
	mRouterAgents	*PoolAndAgent.RouterAgent
	mMyAppid		uint32
	mMapConnection	map[uint64]GateConnection
	mMapUser		map[uint64]GateUserInfo
}
...
//新建一个session客户端
func (this *GateLogic) NetWork_OnConnOK(req *tcpnet.TCPSessionCome) {
	...
	if _, ok := this.mMapConnection[req.Base.ConnId]; ok{
		...
		fmt.Println("gate 新建了一个客户端连接，connId = ", req.Base.ConnId)
		log.Println("gate 新建了一个客户端连接，connId = ", req.Base.ConnId)
...
//收到了客户端传来的消息
func (this *GateLogic) Gate_GateTransferData(req *gate.GateTransferData) {
	...
	routerMsg := new(router.RouterTransferData)
	...
	routerMsg.SrcApptype = uint32(mytype.EnumAppType_Gate)
	routerMsg.DataDirection = router.RouterTransferData_Client2App
...
func (this *GateLogic) Router_OnRouterTransferData(req *router.RouterTransferData) {

	//转发报文,应当只会是那类需要转到客户端的
	//不是转发到客户端的报文，在解析成普通报文后重新推送进自己的mPool中，
	//是否要处理，在processReq入口处决定
	...
	}else if req.DataDirection == router.RouterTransferData_App2App {
...
//登录回复报文
func (this *GateLogic) Client_OnLoginRsp(req *client.LoginRsp) {
	...
	//登录成功的情况下才记录userId
	if userId != 0 && req.LoginResult == client.LoginRsp_SUCCESS {
		//查mMapUser
		userElem, ok := this.mMapUser[userId]
		if ok {
			//如果存在，要做顶号处理，把之前的连接断开，不能一个userId维持两个连接
			kick := new(tcpnet.TCPSessionKick) //userElem.connId就是要断开的connId
...
//收到了客户端的心跳测试请求
func (this *GateLogic) Gate_PulseReq(req *gate.PulseReq) {
	//发送回复
	rsp := new(gate.PulseRsp)
...
func (this *GateLogic) SendToClient(req proto.Message, pBase *mytype.BaseInfo){
	//先把其他报文转成bs_gate.TransferData然后再转成bs_tcp.TCPTransferMsg
	var gateTrans *gate.GateTransferData = nil
